...
import * as jiuXingModule from './jiuxing.mjs';
import * as baMenModule from './bamen.mjs';
import * as baShenModule from './bashen.mjs';
import * as diPanModule from './dipan.mjs';

const { Lunar, Solar, JieQi } = lunarJavascript;
...
/**
 * 将输入转换为 Date（支持 Date 或字符串）
 * @param {Date | String} input 日期时间
 * @returns {Date} 标准 Date 实例
 */
function normalizeDate(input) {
    ...
    if (typeof input !== 'string') {
        throw new Error('Date must be a Date instance or a string.');
    }
...
function calculate(date, options = {}) {
    ...
    try {
        // 获取农历信息
        const lunar = Lunar.fromDate(normalizedDate);
        const solar = Solar.fromDate(normalizedDate);
        ...
        // 计算局数
        const juShu = calculateJuShu(normalizedDate, opts.method);
        ...
        return {
            ...
            message: e.message,
            basicInfo: {
                date: normalizedDate.toLocaleString(),
                type: opts.type,
                method: opts.method,
                purpose: opts.purpose,
                location: opts.location
